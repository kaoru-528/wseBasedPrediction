for(i in seq(1, 4, by = 1)){
    ...
    prediction_term = floor(0.3 * term[[i]])

    if(i == 1){
        name = "./OUTPUT/DS1_2^3_A1_ut_hard_70"
        load(dataPath1)
    }
    else if (i == 2){
        name = "./OUTPUT/DS2_2^3_A1_ut_hard_70"
        load(dataPath2)
    }
    else if (i == 3){
        name = "./OUTPUT/DS3_2^3_A1_ut_hard_70"
        load(dataPath3)
    }
    else if (i == 4){
        name = "./OUTPUT/DS4_2^3_A1_ut_hard_70"
        load(dataPath4)
    }
    print("start", i)
    ...
    run_regression <- function(j) {
        ...
        a_data = data.frame(mse = numeric(), a = numeric(), b = numeric(), c = numeric(), d = numeric())
        coe = list(tmp_Cs_4_1,tmp_Ds_2_1,tmp_Ds_2_2,tmp_Ds_2_3,tmp_Ds_2_4,tmp_Ds_3_1,tmp_Ds_3_2,tmp_Ds_4_1,tmp_dDs_2_1,tmp_dDs_2_2,tmp_dDs_2_3,tmp_dDs_2_4,tmp_dDs_3_1,tmp_dDs_3_2,tmp_dDs_4_1)
        for(sub_a in seq(1, 10, by = 3)){
            for(sub_b in seq(1, 10, by = 3)){
                for(sub_c in seq(1, 10, by = 3)){
                    for(sub_d in seq(0, 10, by = 3)){
                        fit <- nls(unlist(coe[[j]]) ~ f(x, a, b, c, d), start = list(a =  sub_a, b = sub_b, c = sub_c, d = sub_d),control=nls.control(warnOnly=TRUE))
                        params = coef(fit)
                        pre = f(x, params[1], params[2], params[3], params[4])
                        mse = mean((unlist(coe[[j]]) - pre)^2)
    ...
    createGraph(name, data,sorted_best_coe,coe_name,coe_length)
    ...
    best_coe_filename = paste0(name,"_best_coe.xlsx")
    write.xlsx(best_coe,best_coe_filename)
    prediction_filename = paste0(name,".xlsx")
